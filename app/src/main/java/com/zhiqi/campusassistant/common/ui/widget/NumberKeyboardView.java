...
public class NumberKeyboardView extends KeyboardView implements KeyboardView.OnKeyboardActionListener {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.NumberKeyboardView,
                defStyleAttr, 0);
        mDeleteDrawable = a.getDrawable(R.styleable.NumberKeyboardView_deleteDrawable);
        mDeleteBackground = a.getDrawable(R.styleable.NumberKeyboardView_deleteBackground);
        mDeleteWidth = a.getDimensionPixelOffset(R.styleable.NumberKeyboardView_deleteWidth, -1);
        mDeleteHeight = a.getDimensionPixelOffset(R.styleable.NumberKeyboardView_deleteHeight, -1);
        a.recycle();
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (Keyboard.Key key : keys) {
            ...
            if (key.codes[0] == KEYCODE_EMPTY) {
                drawKeyBackground(key, canvas, mDeleteBackground);
            }
    ...
    private void drawKeyBackground(Keyboard.Key key, Canvas canvas, Drawable drawable) {
        boolean topEdge = (key.edgeFlags & Keyboard.EDGE_TOP) > 0;
        int[] drawableState = key.getCurrentDrawableState();
        drawable.setState(drawableState);
        int top = topEdge ? key.y + getPaddingTop() : key.y + key.gap;
        drawable.setBounds(key.x, top, key.x + key.width, key.y + key.height);
