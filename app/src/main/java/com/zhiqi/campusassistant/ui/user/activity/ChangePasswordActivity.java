...
public class ChangePasswordActivity extends BaseToolbarActivity implements IRequestView {
    ...
    void onTextChanged() {
        if (oldPwdEdit.getText().length() < AppConstant.LOGIN_PASSWORD_MIN_LENGTH || newPwdEdit.getText().length() < AppConstant.LOGIN_PASSWORD_MIN_LENGTH ||
    ...
    void onNext() {
        if (!newPwdEdit.getText().toString().equals(confirmEdit.getText().toString())) {
            ...
            mPresenter.changePassword(oldPwdEdit.getText().toString(), newPwdEdit.getText().toString(), this);
        }
    }
