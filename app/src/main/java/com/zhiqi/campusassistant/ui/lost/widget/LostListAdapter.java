...
public class LostListAdapter extends BaseQuickAdapter<LostInfo> {

    public LostListAdapter() {
        super(R.layout.item_lost_info, null);
    }

    @Override
    protected void convert(BaseViewHolder helper, LostInfo item, int position) {
        helper.setText(R.id.user_name, item.user_name);
        helper.setText(R.id.faculty, item.faculty);
        helper.setText(R.id.lost_content, item.content);
        helper.setText(R.id.lost_time, item.publish_time);
        TextView lostName = helper.getView(R.id.lost_name);
        ...
        GlideApp.with(mContext)
                ...
                .into((ImageView) helper.getView(R.id.user_header));
