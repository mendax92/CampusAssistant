...
import com.ming.base.gson.GsonStragegy;
...
public final class GsonUtils {
    ...
    private static SoftReference<ConstructorConstructor> constructorConstructor;

    static {
        GsonBuilder builder = new GsonBuilder();
        builder.addSerializationExclusionStrategy(GsonStragegy.getSerializeIgnoreStrategy())
                .addDeserializationExclusionStrategy(GsonStragegy.getDeserializeIgnoreStrategy())
                .disableHtmlEscaping();
        gson = builder.create();
    }
    ...
    public static String toJson(Object object, Type typeOfT) {
        return getGson().toJson(object, typeOfT);
    }
    ...
    private static <T> ConstructorConstructor getConstructorConstructor() {
        if (constructorConstructor != null) {
            return constructorConstructor.get();
        }
        return null;
    }
     ...
     * @return
     */
    public static <T> ObjectConstructor getObjectConstructor(TypeToken<T> typeToken) {
        if (typeToken == null) {
            return null;
        }
        ConstructorConstructor constructor = getConstructorConstructor();
        if (constructor == null) {
            synchronized (GsonUtils.class) {
                constructor = getConstructorConstructor();
                if (constructor == null) {
                    constructor = new ConstructorConstructor(Collections.<Type, InstanceCreator<?>>emptyMap());
                    constructorConstructor = new SoftReference<>(constructor);
                }
            }
        }
