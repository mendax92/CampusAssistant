...
public abstract class FlexibleDividerDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < validChildCount; i++) {
            ...
            if (Color.TRANSPARENT != background) {
                Rect startRect = new Rect(bounds);
                setDividerStartBound(startRect, groupIndex, parent);
                if (!startRect.equals(bounds)) {
                    Paint paint = getBackgroundPaint(groupIndex, parent);
                    if (startRect.left == startRect.right || startRect.top == startRect.bottom) {
                        c.drawLine(startRect.left, startRect.top, startRect.right, startRect.bottom, paint);
                    } else {
                        c.drawRect(startRect, paint);
                    }
                }
                Rect endRect = new Rect(bounds);
                setDividerEndBound(endRect, groupIndex, parent);
                if (!endRect.equals(bounds)) {
                    Paint paint = getBackgroundPaint(groupIndex, parent);
                    if (endRect.left == endRect.right || endRect.top == endRect.bottom) {
                        c.drawLine(endRect.left, endRect.top, endRect.right, endRect.bottom, paint);
                    } else {
                        c.drawRect(endRect, paint);
                    }
                }
            }
            switch (mDividerType) {
    ...
    private Paint getBackgroundPaint(int position, RecyclerView parent) {
        if (mPaint == null) {
            mPaint = new Paint();
        }
        mPaint.setColor(background);
        mPaint.setStrokeWidth(getDividerSize(position, parent));
        return mPaint;
    }
    ...
    protected abstract void setDividerStartBound(Rect outRect, int position, RecyclerView parent);

    protected abstract void setDividerEndBound(Rect outRect, int position, RecyclerView parent);
    ...
    public static class Builder<T extends Builder> {
        ...
        public T background(@ColorInt int background) {
            this.background = background;
            return (T) this;
        }

        public T backgroundResource(@ColorRes int colorRes) {
            this.background = ContextCompat.getColor(mContext, colorRes);
            return (T) this;
        }
