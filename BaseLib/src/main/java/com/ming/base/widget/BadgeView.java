...
public class BadgeView extends TextView {
    ...
    public static final int POSITION_RIGHT_CENTER = 8;
    ...
    private static final int DEFAULT_EMPTY_SIZE = 8;
    ...
    private int pointSize = DEFAULT_EMPTY_SIZE;
    ...
    private void init(Context context, AttributeSet attrs, View target, int tabIndex) {

        this.context = context;
        this.targetTabIndex = tabIndex;

        TypedArray typeArray = context.obtainStyledAttributes(attrs, R.styleable.badge_view);

        // apply defaults
        badgePosition = typeArray.getInt(R.styleable.badge_view_position, DEFAULT_POSITION);
        int margin = typeArray.getDimensionPixelSize(R.styleable.badge_view_badge_margin, dipToPixels(DEFAULT_MARGIN_DIP));
        badgeMarginH = typeArray.getDimensionPixelSize(R.styleable.badge_view_badge_margin_horizontal, margin);

        badgeMarginV = typeArray.getDimensionPixelSize(R.styleable.badge_view_badge_margin_vertical, margin);

        int paddingPixels = typeArray.getDimensionPixelSize(R.styleable.badge_view_badge_padding, dipToPixels(DEFAULT_LR_PADDING_DIP));

        int paddingV = typeArray.getDimensionPixelSize(R.styleable.badge_view_badge_padding_vertical, 0);
        int paddingH = typeArray.getDimensionPixelSize(R.styleable.badge_view_badge_padding_vertical, paddingPixels);
        setPadding(paddingH, paddingV, paddingH, paddingV);

        badgeColor = typeArray.getColor(R.styleable.badge_view_badge_color, DEFAULT_BADGE_COLOR);

        pointSize = typeArray.getDimensionPixelSize(R.styleable.badge_view_empty_size, dipToPixels(pointSize));

        setTextColor(typeArray.getColor(R.styleable.badge_view_badge_textColor, DEFAULT_TEXT_COLOR));
        setTextSize(typeArray.getDimensionPixelSize(R.styleable.badge_view_badge_textColor, DEFAULT_FONT_SIZE));
    ...
    private void applyLayoutParams() {
        if (target == null) {
            if (TextUtils.isEmpty(getText())) {
                try {
                    ...
                    if (params == null) {
                        params = new ViewGroup.LayoutParams(pointSize, pointSize);
                    } else {
        ...
        if (TextUtils.isEmpty(getText())) {
            lp = new FrameLayout.LayoutParams(pointSize, pointSize);
        } else {
    ...
    /**
     * Set the size of badge background
     *
     * @param size px
     */
    public void setEmplySize(int size) {
        pointSize = size > 0 ? size : dipToPixels(DEFAULT_EMPTY_SIZE);
    }
}
