...
public class MediaStoreHelper {
    ...
    private static class PhotoDirLoaderCallbacks implements LoaderManager.LoaderCallbacks<Cursor> {
        ...
        public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
            ...
            photoDirectoryAll.setName(context.getString(R.string.photo_picker_all_image));
            photoDirectoryAll.setId("ALL");

            while (data.moveToNext()) {

                int imageId = data.getInt(data.getColumnIndexOrThrow(BaseColumns._ID));
                String bucketId = data.getString(data.getColumnIndexOrThrow(ImageColumns.BUCKET_ID));
                String name = data.getString(data.getColumnIndexOrThrow(ImageColumns.BUCKET_DISPLAY_NAME));
                String path = data.getString(data.getColumnIndexOrThrow(MediaColumns.DATA));
                long size = data.getInt(data.getColumnIndexOrThrow(MediaColumns.SIZE));
                ...
                if (!directories.contains(photoDirectory)) {
                    ...
                    photoDirectory.setDateAdded(data.getLong(data.getColumnIndexOrThrow(MediaColumns.DATE_ADDED)));
